...
THREE.OrbitControls = function ( object, domElement ) {
	...
	function onMouseDown( event ) {
		...
		if ( event.button === 0 ) {
			if ( scope.noTilt === true && scope.noRotate === true ) { return; }
	...
	function onMouseMove( event ) {
		...
		if ( state === STATE.ROTATE ) {
			if ( scope.noRotate === true && scope.noTilt === true) { return; }
			
				if(scope.noRotate === false && scope.noTilt === false)
				{					
